		...
		for ($i = 1; $i < $total; $i++ ) {
			...
			foreach ($eventRecords as $eventRecord) {	
				...
				try {
					...
					if (isset($jsonArray['events']['event'][$i]['categories'])) {
						if (isset($jsonArray['events']['event'][$i]['categories']['category'])) {

							$categories = $jsonArray['events']['event'][$i]['categories']['category'];
							...
							for ($cat = 0; $cat < count($categories); $cat++) {
								...
								if (isset($categories['id'])) {
									if ($cat == 0) {
										...
										break; // Don't add more than on category entry for the same category
									}
								} else {
									$category = $categories[$cat];
								}

								$eventfulCategoryID = $category['id'];
								$matchingID = null;

								foreach ($eventfulCategories as $eventfulCategory) {
									if ($eventfulCategory->name == $eventfulCategoryID) {
										$matchingID = $eventfulCategory->id;
									}
								}

								// Eventful Category
								$eventfulEventfulCategory = null;
								$eventfulEventfulCategory = EventfulEventfulCategory::where('eventful_id', '=', $eventRecord->id)->where('eventfulCategories_id', '=', $matchingID)->first();
								...
								if ($eventfulEventfulCategory == null) {
									...
									$eventfulEventfulCategory->eventfulCategories_id = $matchingID;
